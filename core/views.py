...
def index(res):
    default = Defaultdisplay.objects.all()
    ...
    for user in user_following:
        user_list = User.objects.get(username=user.follower)
        user_following_all.append(user_list)

    new_suggestions_list = [x for x in list(all_users) if (x not in list(user_following_all ))]
    current_user = User.objects.filter(username=res.user.username)
    ...
    return render(res, 'index.html',{'user_profile':user_profile,'posts':feed_list,'suggestions_username_profile_list':suggestions_username_profile_list[:4],'default':default})
...
def search(res):
    ...
    return render(res,  'search.html', {'user_profile':user_profile,'username_profile_list':username_profile_list[:4]})
...
def upload(res):
    if res.method == 'POST':
        ...
        file = res.FILES.get('file_upload')
